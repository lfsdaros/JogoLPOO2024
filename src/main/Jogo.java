package main;

import itens.ItemPadrao;
import itens.Item_Energetico;

//import Personagem.PersonagemPadrao;
//import Personagem.Artilharia;
//import Personagem.Cavalaria;
//import Personagem.Infantaria;



public class Jogo {
    public static void main(String[] args) {
        
        // JFrame window = new JFrame();
        // TelaJogo telaJogo = new TelaJogo();
        
        // window.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);  //fechar window qd cliar o botao (X)
        // window.setResizable(false);
        // window.setTitle("JOGO LPOO 2024 - GUERRA");
        // window.add(telaJogo);
        // window.pack();
        // window.setLocationRelativeTo(null);
        // window.setVisible(true); 

        // telaJogo.startGameThread();   //faz a tela aparecer 

        // Cavalaria p3 = new Cavalaria(5, 100);

        // String patente = p3.getPatenteAtual();

        // p3.setDanoRevolver(26);
        // //int danopat = p3.getDanoPatente();
        // int danorev = p3.getDanoRevolver();

        // System.out.println("patente de p1 = "+ patente);
        // System.err.println("dano atual p3 = "+ danorev);

        
        // TESTE
        
       // PersonagemPadrao player1 = new PersonagemPadrao(0);

        
        ItemPadrao item1 = new Item_Energetico(100,100);

        item1.pegarItem(100, 100);
        
    }
}
